...
use URI::SmartURI::URL qw(url);
use URI::SmartURI::Escape qw(uri_escape uri_unescape);

# want compatiblity
use URI::SmartURI::file;
...
$url = new URI::SmartURI::URL "x-myscheme:something";
...
$url = new URI::SmartURI::URL 'x-a+b.c://foo/bar;a?b';
$url->_expect('as_string', 'x-a+b.c://foo/bar;a?b');
$url->_expect('path', '/bar;a?b');
$url->foo;
$newurl = new URI::SmartURI::URL 'xxx', $url;
$newurl->foo;
$url = new URI::SmartURI::URL 'yyy', 'x-foo:';
$url->foo;
...
#		'password'=>'pswd' },
	...
	my $url = new URI::SmartURI::URL $url_str;
...
sub parts_test {
    ...
    $url = new URI::SmartURI::URL 'file://web/orig/path';
...
sub netloc_test {
    ...
    my $url = new URI::SmartURI::URL 'ftp://anonymous:p%61ss@håst:12345';
...
sub port_test {
    ...
    $url = URI::SmartURI::URL->new('http://foo/root/dir/');
    my $port = $url->port;
...
sub escape_test {
    ...
    # supply escaped URL
    $url = new URI::SmartURI::URL 'http://web/this%20has%20spaces';
    ...
    $url = new URI::SmartURI::URL uri_escape('http://web/try %?#" those');
    $url->_expect('as_string',
    ...
    $url = new URI::SmartURI::URL 'file://h/';
    $url->path(chr(0x7F));
    ...
    $url = new URI::SmartURI::URL 'file://h/test?ing';
    ...
    $str = $url->as_string;
    $url = new URI::SmartURI::URL $str;
    die "URL changed" if $str ne $url->as_string;

    $url = new URI::SmartURI::URL 'ftp:foo';
    $url->user($reserved);
    $url->host($reserved);
...
sub newlocal_test {
    ...
    $url = newlocal URI::SmartURI::URL;
    my $ss = $isMSWin32 ? '//' : (($dir =~ m,^/,) ? '' : '///' );
    $url->_expect('as_string', URI::SmartURI::URL->new("file:$ss$dir/")->as_string);
    ...
    # absolute dir
    chdir('/') or die $!;
    $url = newlocal URI::SmartURI::URL '/usr/';
    $url->_expect('as_string', 'file:/usr/');

    # absolute file
    $url = newlocal URI::SmartURI::URL '/vmunix';
    ...
    $dir = uri_escape($dir, ':');
    $dir =~ s/^(\w)%3A/$1:/ if $isMSWin32 or $^O eq 'os2';
    $url = newlocal URI::SmartURI::URL 'foo';
    ...
    $dir = uri_escape($dir, ':');
    $dir =~ s/^(\w)%3A/$1:/ if $isMSWin32 or $^O eq 'os2';
    $url = newlocal URI::SmartURI::URL 'bar/';
...
#    $url->_expect('unix_path', '/');
    ...
    # Relative files
    $url = new URI::SmartURI::URL 'file:foo/b%61r/Note.txt';
    ...
    $url = new URI::SmartURI::URL 'file://vms.host.edu/disk$user/my/notes/note12345.txt';
...
#			  ['%2e%2e/a'  => 'http://a/b/c/%2e%2e/a'],
	...
	my($rel, $abs) = @$test;
	my $abs_url = new URI::SmartURI::URL $abs;
	my $abs_str = $abs_url->as_string;

	printf("  %-10s  +  $base  =>  %s\n", $rel, $abs);
	my $u   = new URI::SmartURI::URL $rel, $base;
	my $got = $u->abs;
	$got->_expect('as_string', $abs_str);
    }

    # bug found and fixed in 1.9 by "J.E. Fritz" <FRITZ@gems.vcu.edu>
    $base = new URI::SmartURI::URL 'http://host/directory/file';
    my $relative = new URI::SmartURI::URL 'file', $base;
    ...
    $base = new URI::SmartURI::URL 'http://host/dir1/../dir2/file';
    $relative = new URI::SmartURI::URL 'file', $base;
    $result = $relative->abs;
...
#	 ["?foo",        "http://abc/a",   "http://abc/a?foo"],
	...
	my $rel = new URI::SmartURI::URL $url, $base;
	...
	my $rel = URI::SmartURI::URL->new($abs, $base)->rel;
...
sub eq_test
{
    my $u1 = new URI::SmartURI::URL 'http://abc.com:80/~smith/home.html';
    my $u2 = new URI::SmartURI::URL 'http://ABC.com/%7Esmith/home.html';
    my $u3 = new URI::SmartURI::URL 'http://ABC.com:/%7esmith/home.html';
    ...
    # Test empty path
    my $u4 = new URI::SmartURI::URL 'http://www.sn.no';
    $u4->eq("HTTP://WWW.SN.NO:80/") or die "7: $u4";
    $u4->eq("http://www.sn.no:81") and die "8: $u4";

    # Test mailto
#    my $u5 = new URI::SmartURI::URL 'mailto:AAS@SN.no';
#    $u5->eq('mailto:aas@sn.no') or die "9: $u5";

    # Test reserved char
    my $u6 = new URI::SmartURI::URL 'ftp://ftp/%2Fetc';
