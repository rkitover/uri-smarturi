...
if (@ARGV && $ARGV[0] eq "store") {
    ...
    require URI::SmartURI::URL;
    my $a = {
        u => new URI::SmartURI('http://search.cpan.org/'),
    };
    print "# store\n";
    store [URI::SmartURI->new("http://search.cpan.org")], 'urls.sto';
} else {
    print "# retrieve\n";
    ...
    print "not " unless ref($u) eq "URI::SmartURI::http";
    print "ok 3\n";
}
